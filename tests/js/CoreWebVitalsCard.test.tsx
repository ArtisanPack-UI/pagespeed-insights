/**
 * The React Core Web Vitals card, against mocked endpoint responses.
 */
...
describe( 'CoreWebVitalsCard', () => {
    ...
    it( 'separates "no field data" from "origin-level data"', async () => {
        renderCard(
            vitalsPayload( { state: 'no-field-data', page: null, origin: null } ),
        )
    ...
    it( 'labels origin-level numbers as describing the site rather than the page', async () => {
        ...
        payload.origin = {
            ...
            vitals: [
                ...
                { metric: 'cumulative_layout_shift', value: 12, band: 'needs-improvement', category: 'AVERAGE' },
            ],
        }
    ...
    it( 'renders the endpoint refusal rather than a blank card', async () => {
        renderCard( { error: 'invalid_url', message: 'Supply a full http:// or https:// address.' }, 422 )

        expect( await screen.findByText( 'This field data could not be loaded' ) ).toBeInTheDocument()
        expect( screen.getByText( 'Supply a full http:// or https:// address.' ) ).toBeInTheDocument()
    } )
} )
