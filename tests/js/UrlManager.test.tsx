/**
 * The React monitored URL manager, against mocked endpoint responses.
 */

import { describe, expect, it } from 'vitest'
import { fireEvent, render, screen, waitFor } from '@testing-library/react'
import { UrlManager } from '../../resources/js/react/UrlManager'
...
describe( 'UrlManager', () => {
    it( 'lists the monitored set', async () => {
        ...
        render( <UrlManager fetchImpl={ fetch.impl } /> )
    ...
    it( 'posts a new URL and reloads the list', async () => {
        ...
        render( <UrlManager fetchImpl={ fetch.impl } csrfToken="token-123" /> )
        ...
        fireEvent.change( screen.getByLabelText( 'Label' ), { target: { value: 'About' } } )
    ...
    it( 'shows the endpoint refusal when a URL cannot be added', async () => {
        ...
        fireEvent.change( screen.getByLabelText( 'URL' ), { target: { value: TEST_URL } } )
        fireEvent.click( screen.getByRole( 'button', { name: 'Add URL' } ) )
    ...
    it( 'renders the endpoint refusal rather than a blank table', async () => {
        ...
        expect( await screen.findByText( 'The monitored set could not be loaded' ) ).toBeInTheDocument()
    } )
} )
