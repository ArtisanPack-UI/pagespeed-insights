/**
 * The React score card, against mocked endpoint responses.
 */

import { describe, expect, it } from 'vitest'
import { fireEvent, render, screen, waitFor } from '@testing-library/react'
import { ScoreCard } from '../../resources/js/react/ScoreCard'
...
function renderCard( body: unknown, status = 200 ) {
    ...
    render( <ScoreCard url={ TEST_URL } fetchImpl={ fetch.impl } /> )
...
describe( 'ScoreCard', () => {
    ...
    it( 'asks the endpoint for the URL and form factor it was given', async () => {
        const fetch = mockFetch( { '/pagespeed/scores': { body: scoresPayload( { strategy: 'desktop' } ) } } )

        render( <ScoreCard url={ TEST_URL } strategy="desktop" fetchImpl={ fetch.impl } /> )
    ...
    it( 'honours an endpointBase override', async () => {
        ...
        render( <ScoreCard url={ TEST_URL } endpointBase="/admin/psi" fetchImpl={ fetch.impl } /> )
    ...
    it( 'reports a failed run with the error the server stored', async () => {
        renderCard(
            scoresPayload( {
                state: 'failed',
                result: resultSummary( { status: 'failed', errorMessage: 'Lighthouse returned an error.' } ),
                scores: [],
                labMetrics: {},
            } ),
        )
    ...
    it( 'separates a degraded run from a clean one and lists its warnings', async () => {
        renderCard(
            scoresPayload( {
                state: 'degraded',
                result: resultSummary( { degraded: true, warnings: [ 'The SEO category was not returned.' ] } ),
            } ),
        )
    ...
    it( 'renders the endpoint refusal rather than a blank card', async () => {
        renderCard( { error: 'url_not_monitored', message: 'This installation does not monitor that URL.' }, 403 )

        expect( await screen.findByText( 'These scores could not be loaded' ) ).toBeInTheDocument()
        expect( screen.getByText( 'This installation does not monitor that URL.' ) ).toBeInTheDocument()
    } )
    ...
    it( 'announces a finished run so the sibling panels stop showing the previous one', async () => {
        ...
        const fetch = mockFetch( {
            ...
            '/pagespeed/test': { status: 202, body: { id: 'ticket-1', status: 'queued', url: TEST_URL, strategy: 'mobile' } },
            '/pagespeed/results/': { body: { id: 42, status: 'completed', url: TEST_URL, strategy: 'mobile', result: null } },
        } )

        render( <ScoreCard url={ TEST_URL } fetchImpl={ fetch.impl } csrfToken="token" /> )
    ...
    it( 'announces the run it queued, not whatever the card is showing when it lands', async () => {
        ...
        const fetch = mockFetch( {
            '/pagespeed/scores': { body: scoresPayload() },
            '/pagespeed/test': { status: 202, body: { id: 'ticket-3', status: 'queued', url: TEST_URL, strategy: 'mobile' } },
            '/pagespeed/results/': { body: { id: 42, status: 'completed', url: TEST_URL, strategy: 'mobile', result: null } },
        } )

        const { rerender } = render(
            <ScoreCard url={ TEST_URL } strategy="mobile" fetchImpl={ fetch.impl } csrfToken="token" />,
        )
        ...
        // new one would refresh three panels for a run that was not theirs,
        // and leave the panels that were waiting on it stale.
        expect( heard[ 0 ] ).toEqual( { url: TEST_URL, strategy: 'mobile', id: 42 } )
    ...
    it( 'announces nothing when the queued run never reports back', async () => {
        ...
        const fetch = mockFetch( {
            '/pagespeed/scores': { body: scoresPayload() },
            '/pagespeed/test': { status: 202, body: { id: 'ticket-2', status: 'queued', url: TEST_URL, strategy: 'mobile' } },
            '/pagespeed/results/': { body: { id: 'ticket-2', status: 'timed-out', url: TEST_URL, strategy: 'mobile', result: null } },
        } )
        ...
        expect( await screen.findByText( 'The test has not reported back', {}, { timeout: 10000 } ) )
            .toBeInTheDocument()
